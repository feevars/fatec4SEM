...
CREATE TABLE `Aula` (
    ...
    CONSTRAINT `fk_cursoId_aulaId` FOREIGN KEY (`cursoId`) REFERENCES `Curso` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE `Exercicio` (
    ...
    `aulaId` INT NOT NULL,
    CONSTRAINT `fk_exercicio_aulaId` FOREIGN KEY (`aulaId`) REFERENCES `Aula`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE `Estudante` (
    ...
    CONSTRAINT `fk_cursoId_instrutorId` FOREIGN KEY (`cursoId`) REFERENCES `Curso`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_instrutorId_cursoId` FOREIGN KEY (`instrutorId`) REFERENCES `Estudante`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE `EstudanteCurso` (
    ...
    CONSTRAINT `fk_estudanteId_cursoId` FOREIGN KEY (`estudanteId`) REFERENCES `Estudante`(`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_cursoId_estudanteId` FOREIGN KEY (`cursoId`) REFERENCES `Curso`(`id`) ON DELETE CASCADE ON UPDATE CASCADE
);
CREATE TABLE `EstudanteExercicio`(
    `estudanteId` INT NOT NULL,
    `exercicioId` INT NOT NULL,
    `acertou` BOOLEAN NOT NULL DEFAULT '0',
    CONSTRAINT `fk_estudanteId_exercicioId` FOREIGN KEY (`estudanteId`) REFERENCES `Estudante` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_exercicioId_estudanteId` FOREIGN KEY (`exercicioId`) REFERENCES `Exercicio` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
);